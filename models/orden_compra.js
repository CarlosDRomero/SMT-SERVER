...
export const ordenCompraModel = {
  ...
  getProductsOrder: async (idorden) => {
    const query = {
      ...
      text: "SELECT p.*, po.costo, po.cantidad FROM producto_orden po JOIN orden_compra oc ON po.idorden = oc.idorden JOIN producto p ON po.idproducto = p.idproducto WHERE po.idorden = $1",
      values: [idorden]
    }
    const res = await poolClient.query(query);
    return res.rows;
